...
void MAC_YourSlotIsr() {
	CRITICAL(
	    int64_t local_time = TRANSCEIVER_GetTime(); uint32_t slot_time = PORT_SlotTimerTickUs(); MAC_UpdateSlotTimer(slot_time, local_time);)
	decaIrqStatus_t en = decamutexon();
...
int MAC_TryTransmitFrameInSlot(int64_t glob_time) {
	...
	if (real_start_us < start_us || end_us < real_start_us || end_us < real_end_us) {
		return 0;
	}
...
void MAC_SetFrameType(mac_buf_t* buf, uint8_t FR_CR_type) {
	MAC_ASSERT(buf != 0);
	MAC_ASSERT(
	    FR_CR_type == FR_CR_ACK || FR_CR_type == FR_CR_BEACON || FR_CR_type == FR_CR_DATA || FR_CR_type == FR_CR_MAC);
