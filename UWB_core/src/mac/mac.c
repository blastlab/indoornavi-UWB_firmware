...
int MAC_ToSlotsTimeUs(int64_t glob_time) {
  int64_t slots_period_dtu =
      (int64_t)settings.mac.slots_sum_time_us /
      (DWT_TIME_UNITS * 1e6f);  // multiple in two lines (conversion 32-64B)
...
void MAC_UpdateSlotTimer(int32_t host_slot_time_us, int64_t uwb_local_time) {
  ...
  PORT_SlotTimerSetUsOffset(
      host_slot_time_us - uwb_slot_time_us);  // host slot time - UWB slot time;
  MAC_TRACE("SYNC %7d %7d %4d", uwb_slot_time_us, host_slot_time_us,
            (int)sync.neighbour[0].drift[0]);
}

// Function called from slot timer interrupt.
void MAC_YourSlotIsr() {
  CRITICAL(int64_t local_time = TRANSCEIVER_GetTime();
...
int MAC_TryTransmitFrameInSlot(int64_t glob_time) {
  ...
  if (tx_est_time >
      settings.mac.slot_time_us - settings.mac.slot_guard_time_us) {
    LOG_WRN("Frame with size %d can't be send within %dus slot", len,
            settings.mac.slot_time_us);
    MAC_Free(buf);
  }
...
void MAC_SetFrameType(mac_buf_t* buf, uint8_t FR_CR_type) {
  MAC_ASSERT(buf != 0);
  MAC_ASSERT(FR_CR_type == FR_CR_ACK || FR_CR_type == FR_CR_BEACON ||
             FR_CR_type == FR_CR_DATA || FR_CR_type == FR_CR_MAC);
  buf->frame.control[0] =
