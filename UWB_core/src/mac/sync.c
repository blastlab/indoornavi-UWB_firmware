...
void sync_init_neightbour(sync_neightbour_t *neig, dev_addr_t addr, int tree_level)
{
...
sync_neightbour_t *sync_find_or_create_neightbour(dev_addr_t addr, int tree_level)
{
...
void sync_update_neightbour(sync_neightbour_t *neig, int64_t ext_time, int64_t loc_time, int tof_dw)
{
    ...
    neig->drift[0] = drift;
    neig->time_coeffP_raw[0] = dt != 0.0 ? ((float)drift) / ((float)dt) : neig->time_coeffP_raw[1];
...
void sync_update(sync_neightbour_t *neig, int64_t ext_time, int64_t loc_time, int tof_dw)
{
