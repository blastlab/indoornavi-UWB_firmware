...
ARG("parent", "hex addres of parent device (in sink direction)")
...
ARG("mV", "battery volatge in millivolts, correct values for devices with battery is 3100..4300, for devices without battery 4500..5200")
ARG("Rx", "12-bits counter of correctly received packets, values in range 0..4095. Aftetr overflow it will count from 0.")
ARG("Tx", "12-bits counter of correctly transmitted packets, values in range 0..4095. Aftetr overflow it will count from 0.")
ARG("Er", "12-bits counter of receiving packets error, values in range 0..4095. Aftetr overflow it will count from 0.")
ARG("To", "12-bits counter of timeout during transmitting or receiving frames, values in range 0..4095. Aftetr overflow it will count from 0.")
ARG("Uptime", "device work time in format days.hours.minuts.seconds. It overflow ofter 49.7 days.")
COMMENT("see :ref:`status`")

ADD_ITEM_M(1112, INF_VERSION, "version did:%X r:%s hV::%d.%d fV:%d.%d.%X")
...
ARG("txpower", "ble transmittes power settings")
