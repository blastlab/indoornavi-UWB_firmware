...
void api_init()
{
	...
	LL_DMA_ConfigTransfer(DW_DMA, DW_DMACH_TX, LL_DMA_DIRECTION_MEMORY_TO_PERIPH|conf);
	LL_DMA_ConfigTransfer(DW_DMA, DW_DMACH_RX, LL_DMA_DIRECTION_PERIPH_TO_MEMORY|conf);

	// set SPI address
	LL_DMA_SetPeriphAddress(DW_DMA, DW_DMACH_TX, (uint32_t)LL_SPI_DMA_GetRegAddr(DW_SPI));
	LL_DMA_SetPeriphAddress(DW_DMA, DW_DMACH_RX, (uint32_t)LL_SPI_DMA_GetRegAddr(DW_SPI));
...
#pragma GCC optimize ("O3")
int spi_write(uint16_t hdrLength, const uint8_t* hdrBuf,
		uint32_t bodyLength, const uint8_t* bodyBuf)
{
...
static inline void spi_tx(uint32_t length, const uint8_t* buf) {
	// configure buffer address and length
	DW_DMACH_TX->CMAR = (uint32_t) buf; // LL_DMA_SetMemoryAddress(DW_DMA, DW_DMACH_TX, (uint32_t)hdrBuf)
	DW_DMACH_TX->CNDTR = length; // LL_DMA_SetDataLength(DW_DMA, DW_DMACH_TX, hdrLength)
	...
	// disable dma channel
	CLEAR_BIT(DW_DMACH_TX->CCR, DMA_CCR_EN); // LL_DMA_DisableChannel(DW_DMA, DW_DMACH_TX);
}

#pragma GCC optimize ("O3")
static inline void spi_rx(uint32_t length, uint8_t* buf) {
	// configure buffer address and length
	DW_DMACH_RX->CMAR = (uint32_t) buf; // LL_DMA_SetMemoryAddress(DW_DMA, DW_DMACH_RX, (uint32_t)buf)
	DW_DMACH_RX->CNDTR = length; // LL_DMA_SetDataLength(DW_DMA, DW_DMACH_RX, length)
	DW_DMACH_TX->CNDTR = length; // LL_DMA_SetDataLength(DW_DMA, DW_DMACH_TX, length)
	...
	// start transmission
	SET_BIT(DW_DMACH_RX->CCR, DMA_CCR_EN);//LL_DMA_EnableChannel(DW_DMA, DW_DMACH_RX);
	SET_BIT(DW_DMACH_TX->CCR, DMA_CCR_EN); //LL_DMA_EnableChannel(DW_DMA, DW_DMACH_TX);
	...
	CLEAR_BIT(DW_DMACH_RX->CCR, DMA_CCR_EN);// LL_DMA_DisableChannel(DW_DMA, DW_DMACH_RX);
	CLEAR_BIT(DW_DMACH_TX->CCR, DMA_CCR_EN); // LL_DMA_DisableChannel(DW_DMA, DW_DMACH_TX);
...
int readfromspi(uint16_t headerLength, const uint8_t* headerBuffer,
		uint32_t bodyLength, uint8_t* bodyBuffer) {
	decaIrqStatus_t en = decamutexon();
	DW_CS_GPIO_Port->BRR = DW_CS_Pin; // LL_GPIO_ResetOutputPin(DW_CS_GPIO_Port, DW_CS_Pin)
	...
	DW_CS_GPIO_Port->BSRR = DW_CS_Pin; // LL_GPIO_SetOutputPin(DW_CS_GPIO_Port, DW_CS_Pin)
	decamutexoff(en);
	return 0;
}
